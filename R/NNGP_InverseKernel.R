...
#
#' @return sigma.inverse, sigma.inverse.det, sigma.det
#' @import foreach 
#'
#' @export
library(foreach)
nngp_approx <- function(y, m, location, kernelMatrix) {
  ...
  for (i in 1:n) {
    ...
    if ((i - m) == (1 - m)) {
      ...
      neighbor.order = match(c(1:(i-1)),order(dist.si[i,1:(i - 1)]))
      neighbors.si = ordered.loc[neighbor.order,]
      ...
      neighbor.order = match(c(1:m),order(dist.si[i,1:(i - 1)]))
      neighbors.si = ordered.loc[neighbor.order,]
  ...
  return(list(sigma.inverse = sigma.inverse, sigma.inverse.det = sum(1/F.vector), sigma.det = sum(F.vector)))
}
