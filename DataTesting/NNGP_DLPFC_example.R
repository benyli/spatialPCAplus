...
library(glmGamPoi)
library(bluster)
library(SpatialPCAplus)

source("NNGP_InverseKernel.R")
...
LIBD = CreateSpatialPCAObject(counts=count_sub, location=xy_coords, project = "SpatialPCA",gene.type="spatial",sparkversion="spark",numCores_spark=1,gene.number=3000, customGenelist=NULL,min.loctions = 20, min.features=20)
...
# NNGP method
nngpOutput = nngp_approx(y = count_sub, m = 25, location = t(location),kernelMatrix = LIBD@kernelmat)
...
system.time(nngp_approx(y = rawcount, m = 25, location = t(location),kernelMatrix = LIBD@kernelmat))
